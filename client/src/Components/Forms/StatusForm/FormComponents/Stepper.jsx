    ...
    const stepsDisplay = newStep.map((step, index) => {
        return (
            ...
            >
                <div className="relative flex flex-col items-center text-teal-600">
                    <div
                        className={`rounded-full transition duration-500 ease-in-out border-2 border-gray-300 h-12 w-12 flex items-center justify-center py-3  ${step.selected
                            ? "bg-indigo-700 text-white font-bold border border-indigo-700 "
                            : ""
                            }`}
                    >
                        {step.completed ? (
                ...
                <div
                    className={`flex-auto border-t-2 transition duration-500 ease-in-out  ${step.completed ? "border-indigo-700" : "border-gray-300 "
                        }  `}
                ></div>
            </div>
        );
